...
#define SIZE_X				 800//640		// 設定遊戲畫面的解析度為640x480
#define SIZE_Y				 600//480		// 註：若不使用標準的解析度，則不能切換到全螢幕
...
#define ENABLE_GAME_PAUSE	 false//true		// 是否允許以 Ctrl-Q 暫停遊戲
...
private:
	...
	static void  BltBitmapToBack(unsigned SurfaceID, int x, int y, double factorX, double factorY);
...
public:
	...
	void  ShowBitmap(double factorX,double factorY);
...
private:
	...
	MUSIC_TIME		 				   GetTimeOffset(MUSIC_TIME,MUSIC_TIME,MUSIC_TIME,MUSIC_TIME,MUSIC_TIME,MUSIC_TIME,DWORD);
	IDirectMusicLoader8				   *g_pLoader;
	IDirectMusicPerformance8		   *g_pPerformance;
	vector<IDirectMusicSegment8 *>	   g_pSegment;
	vector<IDirectMusicSegmentState *> g_pSegmentState;
	vector<long>					   g_ResumePoint;
	vector<bool>					   g_isPaused;
	vector<bool>					   isMIDI;
